...
class Sidebar(ctk.CTkFrame):
    def __init__(self, master, on_filter_change, on_folder_change, **kwargs):
        ...
        self.kind_menu = ctk.CTkOptionMenu(self, values=["all", "image", "video"], variable=self.kind_var, command=lambda _v: on_filter_change())
        self.kind_menu.grid(row=1, column=0, sticky="ew", padx=10)

        self.favorite_var = ctk.BooleanVar(value=False)
        self.favorite_check = ctk.CTkCheckBox(self, text="仅收藏", variable=self.favorite_var, command=on_filter_change)
        ...
        self.folder_menu = ctk.CTkOptionMenu(self, values=[""], variable=self.folder_var, command=lambda _v: on_folder_change())
